  ...
  return (
    <IonPage>
      ...
      <IonContent>
        ...
        {items && (
          <IonList>
            {items.map(({ id, title, message, done, lastChange, characters }) => (
              <Item key={id} id={id} title={title} message={message} done={done} lastChange={lastChange} characters={characters} onEdit={(id) => history.push(`/item/${id}`)} />
            ))}
          </IonList>
        )}
